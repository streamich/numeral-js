...
exports.byteUnits = {
    bytesDecimal: function (test) {
        ...
        for (i = 0; i < tests.length; ++i) {
            test.strictEqual(numeral(tests[i][0]).byteUnits(true), tests[i][1], tests[i][0] + ' ' + tests[i][1]);
        }

        test.done();
    },

    bytesBinary: function (test) {
        ...
        for (i = 0; i < tests.length; ++i) {
            test.strictEqual(numeral(tests[i][0]).byteUnits(false), tests[i][1], tests[i][0] + ' ' + tests[i][1]);
        }
